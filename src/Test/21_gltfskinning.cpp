...
//-----------------------------------------------------------------------------
void GLTFSkinningApp::OnDestroy()
{
...
//-----------------------------------------------------------------------------
void GLTFSkinningApp::OnUpdateUIOverlay(vks::UIOverlay* overlay)
{
...
//-----------------------------------------------------------------------------
void GLTFSkinningApp::OnWindowResize(uint32_t destWidth, uint32_t destHeight)
{
	if ((destWidth > 0.0f) && (destHeight > 0.0f))
		camera.updateAspectRatio((float)destWidth / (float)destHeight);
}
//-----------------------------------------------------------------------------
void GLTFSkinningApp::OnKeyPressed(uint32_t key)
{
	if (key == KEY_F2)
	{
...
//-----------------------------------------------------------------------------
void GLTFSkinningApp::OnKeyUp(uint32_t key)
{
	if (camera.type == Camera::firstperson)
	{
		switch (key)
...
//-----------------------------------------------------------------------------
void GLTFSkinningApp::OnMouseMoved(int32_t x, int32_t y, int32_t dx, int32_t dy)
{
 ...
 */
  ...
  */
glm::mat4 VulkanglTFModel2::Node::getLocalMatrix()
{
...
*/
VulkanglTFModel2::~VulkanglTFModel2()
{
...
void VulkanglTFModel2::loadImages(tinygltf::Model& input)
{
...
void VulkanglTFModel2::loadTextures(tinygltf::Model& input)
{
...
void VulkanglTFModel2::loadMaterials(tinygltf::Model& input)
{
...
VulkanglTFModel2::Node* VulkanglTFModel2::findNode(Node* parent, uint32_t index)
{
...
VulkanglTFModel2::Node* VulkanglTFModel2::nodeFromIndex(uint32_t index)
{
	Node* nodeFound = nullptr;
	for (auto& node : nodes)
	{
...
void VulkanglTFModel2::loadSkins(tinygltf::Model& input)
{
...
void VulkanglTFModel2::loadAnimations(tinygltf::Model& input)
{
...
void VulkanglTFModel2::loadNode(const tinygltf::Node& inputNode, const tinygltf::Model& input, VulkanglTFModel2::Node* parent, uint32_t nodeIndex, std::vector<uint32_t>& indexBuffer, std::vector<VulkanglTFModel2::Vertex>& vertexBuffer)
{
	VulkanglTFModel2::Node* node = new VulkanglTFModel2::Node{};
	node->parent = parent;
...
glm::mat4 VulkanglTFModel2::getNodeMatrix(VulkanglTFModel2::Node* node)
{
	glm::mat4              nodeMatrix = node->getLocalMatrix();
	VulkanglTFModel2::Node* currentParent = node->parent;
	while (currentParent)
	{
...
void VulkanglTFModel2::updateJoints(VulkanglTFModel2::Node* node)
{
	if (node->skin > -1)
	{
...
void VulkanglTFModel2::updateAnimation(float deltaTime)
{
...
void VulkanglTFModel2::drawNode(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout, VulkanglTFModel2::Node node)
{
	if (node.mesh.primitives.size() > 0)
	{
		...
		for (VulkanglTFModel2::Primitive& primitive : node.mesh.primitives)
		{
			if (primitive.indexCount > 0)
			{
				// Get the texture index for this primitive
				VulkanglTFModel2::Texture texture = textures[materials[primitive.materialIndex].baseColorTextureIndex];
...
void VulkanglTFModel2::draw(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout)
{
...
void GLTFSkinningApp::buildCommandBuffers()
{
	...
	for (int32_t i = 0; i < drawCommandBuffers.size(); ++i)
	{
		...
		vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, wireframe ? pipelines.wireframe : pipelines.solid);
		glTFModel.draw(drawCommandBuffers[i], pipelineLayout);
...
void GLTFSkinningApp::loadglTFFile(std::string filename)
{
	...
	glTFModel.vulkanDevice = m_vulkanDevice;
	glTFModel.copyQueue = queue;

	std::vector<uint32_t>                indexBuffer;
	std::vector<VulkanglTFModel2::Vertex> vertexBuffer;
	...
	size_t vertexBufferSize = vertexBuffer.size() * sizeof(VulkanglTFModel2::Vertex);
...
void GLTFSkinningApp::setupDescriptors()
{
	/*
...
void GLTFSkinningApp::preparePipelines()
{
	// Layout
	...
	const std::vector<VkVertexInputBindingDescription> vertexInputBindings = {
		vks::initializers::vertexInputBindingDescription(0, sizeof(VulkanglTFModel2::Vertex), VK_VERTEX_INPUT_RATE_VERTEX),
	};
	const std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
		{0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(VulkanglTFModel2::Vertex, pos)},
		{1, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(VulkanglTFModel2::Vertex, normal)},
		{2, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(VulkanglTFModel2::Vertex, uv)},
		{3, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(VulkanglTFModel2::Vertex, color)},
		// POI: Per-Vertex Joint indices and weights are passed to the vertex shader
		{4, 0, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(VulkanglTFModel2::Vertex, jointIndices)},
		{5, 0, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(VulkanglTFModel2::Vertex, jointWeights)},
	};
...
void GLTFSkinningApp::prepareUniformBuffers()
{
...
void GLTFSkinningApp::updateUniformBuffers()
{
...
void GLTFSkinningApp::loadAssets()
{
