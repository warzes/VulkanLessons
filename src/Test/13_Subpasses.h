...
class SubpassesApp final : public EngineApp
{
public:
...
private:
	...
	{
		attachments.width = destWidth;
		attachments.height = destHeight;
	...
	void buildCommandBuffers() final
	{
		...
		for (int32_t i = 0; i < drawCommandBuffers.size(); ++i)
		{
			...
			{
				vks::debugutils::CmdBeginLabel(drawCommandBuffers[i], "Subpass 0: Deferred G-Buffer creation", { 1.0f, 0.78f, 0.05f, 1.0f });

				vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.offscreen);
				vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.offscreen, 0, 1, &descriptorSets.scene, 0, NULL);
			...
			{
				vks::debugutils::CmdBeginLabel(drawCommandBuffers[i], "Subpass 1: Deferred composition", { 0.0f, 0.5f, 1.0f, 1.0f });
			...
			{
				vks::debugutils::CmdBeginLabel(drawCommandBuffers[i], "Subpass 2: Forward transparency", { 0.5f, 0.76f, 0.34f, 1.0f });

				vkCmdNextSubpass(drawCommandBuffers[i], VK_SUBPASS_CONTENTS_INLINE);

				vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.transparent);
				vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.transparent, 0, 1, &descriptorSets.transparent, 0, NULL);
				models.transparent.draw(drawCommandBuffers[i]);
	...
	void loadAssets()
	{
		...
		models.scene.loadFromFile(getAssetPath() + "models/samplebuilding.gltf", m_vulkanDevice, queue, glTFLoadingFlags);
		models.transparent.loadFromFile(getAssetPath() + "models/samplebuilding_glass.gltf", m_vulkanDevice, queue, glTFLoadingFlags);
		textures.glass.loadFromFile(getAssetPath() + "textures/colored_glass_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, m_vulkanDevice, queue);
	}
	...
	{
		// Matrices
		m_vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &buffers.GBuffer, sizeof(uboGBuffer));
		VK_CHECK_RESULT(buffers.GBuffer.map());

		// Lights
		m_vulkanDevice->createBuffer(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &buffers.lights, lights.size() * sizeof(Light));
