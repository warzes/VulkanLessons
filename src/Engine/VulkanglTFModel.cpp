...
//#define TINYGLTF_NO_EXTERNAL_IMAGE // TODO: требуется для 14_Offscreen/22_gltfscenerendering
	...
	if (node.mesh > -1) {
		...
		for (size_t j = 0; j < mesh.primitives.size(); j++) {
			...
			// Indices
			{
				...
				default:
					Fatal("Index component type " + std::to_string(accessor.componentType) + " not supported!");
					return;
				}
			}
	...
	for (tinygltf::Animation& anim : gltfModel.animations) {
		...
		for (auto& source : anim.channels) {
			...
			if (source.target_path == "weights") {
				Fatal("weights not yet supported, skipping channel");
				continue;
			}
...
void vkglTF::Model::loadFromFile(std::string filename, VulkanDevice* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	for (auto extension : gltfModel.extensionsUsed) {
		if (extension == "KHR_materials_pbrSpecularGlossiness") {
			Fatal("Required extension: " + extension);
...
void vkglTF::Model::updateAnimation(uint32_t index, float time)
{
	if (index > static_cast<uint32_t>(animations.size()) - 1) {
		Fatal("No animation with index " + std::to_string(index));
		return;
	}
