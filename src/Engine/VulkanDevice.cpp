...
//-----------------------------------------------------------------------------
VkResult VulkanDevice::CreateLogicalDevice(VkPhysicalDeviceFeatures inEnabledFeatures, std::vector<const char*> enabledExtensions, void* pNextChain, bool useSwapChain, VkQueueFlags requestedQueueTypes)
{
	...
	VkPhysicalDeviceFeatures2 physicalDeviceFeatures2{ .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2 };
	if (pNextChain)
	{
	...
	if (deviceExtensions.size() > 0)
	{
		for (const char* enabledExtension : deviceExtensions)
		{
			...
			{
				Fatal("Enabled device extension '" + std::string(enabledExtension) + "' is not present at device level");
			}
		}
...
*/
uint32_t VulkanDevice::getMemoryType(uint32_t typeBits, VkMemoryPropertyFlags inProperties, VkBool32* memTypeFound) const
{
