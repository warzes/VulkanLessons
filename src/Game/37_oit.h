...
private:
	...
	void loadAssets()
	{
		...
		models.cube.loadFromFile(getAssetPath() + "models/cube.gltf", m_vulkanDevice, queue, glTFLoadingFlags);
	}

	void prepareUniformBuffers()
	{
		// Create an uniform buffer for a render pass.
		VK_CHECK_RESULT(m_vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &renderPassUniformBuffer, sizeof(RenderPassUniformData)));
	...
	void prepareGeometryPass()
	{
		...
		fbufCreateInfo.width = destWidth;
		fbufCreateInfo.height = destHeight;
		...
		geometrySBO.count = 0;
		geometrySBO.maxNodeCount = NODE_COUNT * destWidth * destHeight;
		...
		geometryPass.headIndex.device = m_vulkanDevice;

		VkImageCreateInfo imageInfo = vks::initializers::imageCreateInfo();
		imageInfo.imageType = VK_IMAGE_TYPE_2D;
		imageInfo.format = VK_FORMAT_R32_UINT;
		imageInfo.extent.width = destWidth;
		imageInfo.extent.height = destHeight;
		...
		geometryPass.headIndex.width = destWidth;
		geometryPass.headIndex.height = destHeight;
