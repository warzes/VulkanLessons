...
class TextOverlayApp final : public EngineApp
{
public:
...
private:
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCommandBuffers.size(); ++i)
		{
			...
			textOverlay->draw(drawCommandBuffers[i]);
	...
	void updateTextOverlay(void)
	{
		...
		textOverlay->addText("TEXT TEST", 5.0f * GetUIOverlay().scale, 5.0f * GetUIOverlay().scale, TextOverlay::alignLeft);
		...
		for (uint32_t i = 0; i < 4; i++)
		{
			...
			textOverlay->addText(ss.str(), (float)destWidth - 5.0f * GetUIOverlay().scale, (25.0f + (float)i * 20.0f) * GetUIOverlay().scale, TextOverlay::alignRight);
		}

		glm::vec3 projected = glm::project(glm::vec3(0.0f), uniformData.modelView, uniformData.projection, glm::vec4(0, 0, (float)destWidth, (float)destHeight));
		textOverlay->addText("A torus knot", projected.x, projected.y, TextOverlay::alignCenter);

#if defined(__ANDROID__)
#else
		textOverlay->addText("Press \"space\" to toggle text overlay", 5.0f * GetUIOverlay().scale, 65.0f * GetUIOverlay().scale, TextOverlay::alignLeft);
		textOverlay->addText("Hold middle mouse button and drag to move", 5.0f * GetUIOverlay().scale, 85.0f * GetUIOverlay().scale, TextOverlay::alignLeft);
#endif
	...
	void loadAssets()
	{
		...
		model.loadFromFile(getAssetPath() + "models/torusknot.gltf", m_vulkanDevice, queue, glTFLoadingFlags);
	}
