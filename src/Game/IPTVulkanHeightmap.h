...
namespace vks
{
	...
	public:
		...
		HeightMap(VulkanDevice* device, VkQueue copyQueue)
		{
			this->device = device;
...
//					data[x + y * texture.width] = (data[x + y * texture.width] + 1.0f) / (2.0f * maxPossibleNoiseHeight / 1.5f);
		...
		void generateMesh(glm::vec3 scale, Topology topology, int levelOfDetail)
		{
			...
			VkCommandBuffer copyCmd = createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true, VK_QUEUE_TRANSFER_BIT);
			...
			flushCommandBuffer(copyCmd, copyQueue, true, VK_QUEUE_TRANSFER_BIT);
		...
		// TODO: скопировал с старого кода (device->createCommandBuffer)
		...
		*/
		...
		{
			VkCommandPool cmdPool = device->commandPool;
			/*if (queueType == VK_QUEUE_TRANSFER_BIT) {
				cmdPool = device->commandPoolTransfer;
			}*/
