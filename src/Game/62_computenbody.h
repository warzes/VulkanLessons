...
class ComputeNBodyApp final : public EngineApp
{
public:
...
private:
	...
	void prepareStorageBuffers()
	{
		...
		m_vulkanDevice->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &storageBuffer, storageBufferSize);
	...
	void prepareGraphics()
	{
		...
		m_vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &graphics.uniformBuffer, sizeof(Graphics::UniformData));
	...
	void prepareCompute()
	{
		...
		uint32_t sharedDataSize = std::min((uint32_t)1024, (uint32_t)(m_vulkanDevice->properties.limits.maxComputeSharedMemorySize / sizeof(glm::vec4)));
	...
	void updateGraphicsUniformBuffers()
	{
		...
		graphics.uniformData.screenDim = glm::vec2((float)destWidth, (float)destHeight);
