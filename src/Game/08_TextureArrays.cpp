...
//-----------------------------------------------------------------------------
bool TextureArraysApp::OnCreate()
{
...
//-----------------------------------------------------------------------------
void TextureArraysApp::OnDestroy()
{
	if (device) {
...
//-----------------------------------------------------------------------------
void TextureArraysApp::OnWindowResize(uint32_t destWidth, uint32_t destHeight)
{
	if ((destWidth > 0.0f) && (destHeight > 0.0f))
		camera.updateAspectRatio((float)destWidth / (float)destHeight);
}
//-----------------------------------------------------------------------------
void TextureArraysApp::OnKeyPressed(uint32_t key)
{
	if (key == KEY_F2)
	{
...
//-----------------------------------------------------------------------------
void TextureArraysApp::OnKeyUp(uint32_t key)
{
	if (camera.type == Camera::firstperson)
	{
		switch (key)
...
//-----------------------------------------------------------------------------
void TextureArraysApp::OnMouseMoved(int32_t x, int32_t y, int32_t dx, int32_t dy)
{
...
//-----------------------------------------------------------------------------
void TextureArraysApp::loadTextureArray(std::string filename, VkFormat format)
{
	ktxResult result;
...
void TextureArraysApp::buildCommandBuffers()
{
	...
	for (int32_t i = 0; i < drawCommandBuffers.size(); ++i)
	{
		...
		vkCmdDrawIndexed(drawCommandBuffers[i], indexCount, layerCount, 0, 0, 0);
...
//-----------------------------------------------------------------------------
void TextureArraysApp::generateCube()
{
...
//-----------------------------------------------------------------------------
void TextureArraysApp::setupDescriptors()
{
	// Pool
...
//-----------------------------------------------------------------------------
void TextureArraysApp::preparePipelines()
{
	// Layout
...
//-----------------------------------------------------------------------------
void TextureArraysApp::prepareUniformBuffers()
{
	...
	VK_CHECK_RESULT(m_vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffer, uboSize));
...
//-----------------------------------------------------------------------------
void TextureArraysApp::updateUniformBuffersCamera()
{
