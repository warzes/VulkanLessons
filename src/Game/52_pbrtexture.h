...
class PBRTextureApp final : public EngineApp
{
public:
...
private:
	...
	void buildCommandBuffers()
	{
		...
		for (size_t i = 0; i < drawCommandBuffers.size(); ++i)
		{
			...
			if (displaySkybox)
			{
				vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets.skybox, 0, NULL);
			...
			// Objects
			vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets.object, 0, NULL);
			vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.pbr);
			models.object.draw(drawCommandBuffers[i]);
	...
	void loadAssets()
	{
		...
		models.object.loadFromFile(getAssetPath() + "models/cerberus/cerberus.gltf", m_vulkanDevice, queue, glTFLoadingFlags);
		textures.environmentCube.loadFromFile(getAssetPath() + "textures/hdr/gcanyon_cube.ktx", VK_FORMAT_R16G16B16A16_SFLOAT, m_vulkanDevice, queue);
		textures.albedoMap.loadFromFile(getAssetPath() + "models/cerberus/albedo.ktx", VK_FORMAT_R8G8B8A8_UNORM, m_vulkanDevice, queue);
		textures.normalMap.loadFromFile(getAssetPath() + "models/cerberus/normal.ktx", VK_FORMAT_R8G8B8A8_UNORM, m_vulkanDevice, queue);
		textures.aoMap.loadFromFile(getAssetPath() + "models/cerberus/ao.ktx", VK_FORMAT_R8_UNORM, m_vulkanDevice, queue);
		textures.metallicMap.loadFromFile(getAssetPath() + "models/cerberus/metallic.ktx", VK_FORMAT_R8_UNORM, m_vulkanDevice, queue);
		textures.roughnessMap.loadFromFile(getAssetPath() + "models/cerberus/roughness.ktx", VK_FORMAT_R8_UNORM, m_vulkanDevice, queue);
	}
	...
	void generateBRDFLUT()
	{
		...
		textures.lutBrdf.device = m_vulkanDevice;
		...
		m_vulkanDevice->flushCommandBuffer(cmdBuf, queue);
	...
	void generateIrradianceCube()
	{
		...
		textures.irradianceCube.device = m_vulkanDevice;
	...
	void generatePrefilteredCube()
	{
		...
		textures.prefilteredCube.device = m_vulkanDevice;
