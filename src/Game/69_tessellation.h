...
private:
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCommandBuffers.size(); ++i)
		{
			...
			if (splitScreen) {
				vkCmdSetViewport(drawCommandBuffers[i], 0, 1, &viewport);
				vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, wireframe ? pipelines.wirePassThrough : pipelines.solidPassThrough);
				model.draw(drawCommandBuffers[i], vkglTF::RenderFlags::BindImages, pipelineLayout);
				viewport.x = float(destWidth) / 2;
			}

			vkCmdSetViewport(drawCommandBuffers[i], 0, 1, &viewport);
			vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, wireframe ? pipelines.wire : pipelines.solid);
			model.draw(drawCommandBuffers[i], vkglTF::RenderFlags::BindImages, pipelineLayout);
