...
class TextureSparseResidencyApp final : public EngineApp
{
public:
...
private:
	...
	{
		texture.device = m_vulkanDevice->logicalDevice;
		texture.width = width;
		...
		vkGetPhysicalDeviceSparseImageFormatProperties(m_physicalDevice, format, imageType, sampleCount, imageUsage, imageTiling, &sparsePropertiesCount, nullptr);
		...
		vkGetPhysicalDeviceSparseImageFormatProperties(m_physicalDevice, format, imageType, sampleCount, imageUsage, imageTiling, &sparsePropertiesCount, sparseProperties.data());
		...
		m_vulkanDevice->flushCommandBuffer(copyCmd, queue);
		...
		if (sparseImageMemoryReqs.size > m_vulkanDevice->properties.limits.sparseAddressSpaceSize)
		{
		...
		texture.memoryTypeIndex = m_vulkanDevice->getMemoryType(sparseImageMemoryReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		...
		sampler.maxAnisotropy = m_vulkanDevice->features.samplerAnisotropy ? m_vulkanDevice->properties.limits.maxSamplerAnisotropy : 1.0f;
