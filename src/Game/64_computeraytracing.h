...
class ComputeRayTracingApp final : public EngineApp
{
public:
...
private:
	...
	void prepareStorageImage()
	{
		...
		if (m_vulkanDevice->queueFamilyIndices.graphics != m_vulkanDevice->queueFamilyIndices.compute)
		{
			...
			imageMemoryBarrier.srcQueueFamilyIndex = m_vulkanDevice->queueFamilyIndices.graphics;
			imageMemoryBarrier.dstQueueFamilyIndex = m_vulkanDevice->queueFamilyIndices.compute;
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCommandBuffers.size(); ++i)
		{
			...
			if (m_vulkanDevice->queueFamilyIndices.graphics != m_vulkanDevice->queueFamilyIndices.compute)
			{
				...
				imageMemoryBarrier.srcQueueFamilyIndex = m_vulkanDevice->queueFamilyIndices.compute;
				imageMemoryBarrier.dstQueueFamilyIndex = m_vulkanDevice->queueFamilyIndices.graphics;
				vkCmdPipelineBarrier(
			...
			vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphics.pipelineLayout, 0, 1, &graphics.descriptorSet, 0, NULL);
	...
	void prepareCompute()
	{
		...
		queueCreateInfo.queueFamilyIndex = m_vulkanDevice->queueFamilyIndices.compute;
	...
	void prepareUniformBuffers()
	{
		...
		m_vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &compute.uniformBuffer, sizeof(Compute::UniformDataCompute));
	}

	void updateUniformBuffers()
	{
		compute.uniformData.aspectRatio = (float)destWidth / (float)destHeight;
