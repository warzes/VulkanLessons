...
class ScreenshotApp final : public EngineApp
{
public:
...
private:
	...
	void prepareUniformBuffers()
	{
		...
		m_vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffer, sizeof(UniformData));
	...
	void saveScreenshot(const char* filename)
	{
		...
		vkGetPhysicalDeviceFormatProperties(m_physicalDevice, swapChain.colorFormat, &formatProps);
		...
		imageCreateCI.extent.width = destWidth;
		imageCreateCI.extent.height = destHeight;
		...
		if (supportsBlit)
		{
			...
			VkOffset3D blitSize;
			blitSize.x = destWidth;
			blitSize.y = destHeight;
			blitSize.z = 1;
		...
		else
		{
			...
			imageCopyRegion.extent.width = destWidth;
			imageCopyRegion.extent.height = destHeight;
