...
class SSAOApp final : public EngineApp
{
public:
...
private:
	...
	void prepareOffscreenFramebuffers()
	{
...
#else
		const uint32_t ssaoWidth = destWidth;
		const uint32_t ssaoHeight = destHeight;
#endif

		frameBuffers.offscreen.setSize(destWidth, destHeight);
		frameBuffers.ssao.setSize(ssaoWidth, ssaoHeight);
		frameBuffers.ssaoBlur.setSize(destWidth, destHeight);
		...
		// G-Buffer
		createAttachment(VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.offscreen.position, destWidth, destHeight);	// Position + Depth
		createAttachment(VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.offscreen.normal, destWidth, destHeight);			// Normals
		createAttachment(VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.offscreen.albedo, destWidth, destHeight);			// Albedo (color)
		createAttachment(attDepthFormat, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, &frameBuffers.offscreen.depth, destWidth, destHeight);			// Depth
		...
		// SSAO blur
		createAttachment(VK_FORMAT_R8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.ssaoBlur.color, destWidth, destHeight);					// Color
	...
	void loadAssets()
	{
		...
		scene.loadFromFile(getAssetPath() + "models/sponza/sponza.gltf", m_vulkanDevice, queue, gltfLoadingFlags);
	}
			...
			*/
			{
				...
				*/
				...
				vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.gBuffer, 0, 1, &descriptorSets.gBuffer, 0, nullptr);
				scene.draw(drawCommandBuffers[i], vkglTF::RenderFlags::BindImages, pipelineLayouts.gBuffer);
				...
				*/
				...
				vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.ssao, 0, 1, &descriptorSets.ssao, 0, nullptr);
				vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.ssao);
				...
				*/
				...
				vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.ssaoBlur, 0, 1, &descriptorSets.ssaoBlur, 0, nullptr);
				vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.ssaoBlur);
				vkCmdDraw(drawCommandBuffers[i], 3, 1, 0, 0);
			...
			*/
			{
				...
				renderPassBeginInfo.framebuffer = EngineApp::frameBuffers[i];
				...
				vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.composition, 0, 1, &descriptorSets.composition, 0, NULL);

				// Final composition pass
				vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.composition);
				vkCmdDraw(drawCommandBuffers[i], 3, 1, 0, 0);
	...
	void prepareUniformBuffers()
	{
		...
		// Upload as texture
		ssaoNoise.fromBuffer(noiseValues.data(), noiseValues.size() * sizeof(glm::vec4), VK_FORMAT_R32G32B32A32_SFLOAT, SSAO_NOISE_DIM, SSAO_NOISE_DIM, m_vulkanDevice, queue, VK_FILTER_NEAREST);
	}
