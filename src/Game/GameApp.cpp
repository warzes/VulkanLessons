...
void GameApp::OnDestroy()
{
	...
	vkDestroyPipeline(GetRenderSystem()->GetDevice(), pipeline, nullptr);

	vkDestroyPipelineLayout(GetRenderSystem()->GetDevice(), pipelineLayout, nullptr);
	vkDestroyDescriptorSetLayout(GetRenderSystem()->GetDevice(), descriptorSetLayout, nullptr);

	vkDestroyBuffer(GetRenderSystem()->GetDevice(), vertices.buffer, nullptr);
	vkFreeMemory(GetRenderSystem()->GetDevice(), vertices.memory, nullptr);

	vkDestroyBuffer(GetRenderSystem()->GetDevice(), indices.buffer, nullptr);
	vkFreeMemory(GetRenderSystem()->GetDevice(), indices.memory, nullptr);

	vkDestroyCommandPool(GetRenderSystem()->GetDevice(), commandPool, nullptr);

	for (uint32_t i = 0; i < MAX_CONCURRENT_FRAMES; i++)
	{
		vkDestroyFence(GetRenderSystem()->GetDevice(), waitFences[i], nullptr);
		vkDestroySemaphore(GetRenderSystem()->GetDevice(), presentCompleteSemaphores[i], nullptr);
		vkDestroySemaphore(GetRenderSystem()->GetDevice(), renderCompleteSemaphores[i], nullptr);
		vkDestroyBuffer(GetRenderSystem()->GetDevice(), uniformBuffers[i].buffer, nullptr);
		vkFreeMemory(GetRenderSystem()->GetDevice(), uniformBuffers[i].memory, nullptr);
	}
}
//-----------------------------------------------------------------------------
...
void GameApp::createPipelines()
{
	...
	shaderStages[0].module = loadSPIRVShader("triangle/triangle.vert.spv");
	...
	// Load binary SPIR-V shader
	shaderStages[1].module = loadSPIRVShader("triangle/triangle.frag.spv");
	// Main entry point for the shader
