...
private:
	...
	void buildCommandBuffers()
	{
		...
		for (size_t i = 0; i < drawCommandBuffers.size(); ++i)
		{
			...
			for (uint32_t x = 0; x < objcount; x++) {
				...
				vkCmdPushConstants(drawCommandBuffers[i], pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(glm::vec3), sizeof(Material::PushBlock), &mat);
	...
	void loadAssets()
	{
		...
		// HDR cubemap
		textures.environmentCube.loadFromFile(getAssetPath() + "textures/hdr/pisa_cube.ktx", VK_FORMAT_R16G16B16A16_SFLOAT, m_vulkanDevice, queue);
	}
