...
class DynamicRenderingApp final : public EngineApp
{
public:
...
private:
	...
	void loadAssets()
	{
		...
		model.loadFromFile(getAssetPath() + "models/voyager.gltf", m_vulkanDevice, queue, glTFLoadingFlags);
	}
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCommandBuffers.size(); ++i)
		{
			...
			renderingInfo.renderArea = { 0, 0, destWidth, destHeight };
			...
			vkCmdBeginRenderingKHR(drawCommandBuffers[i], &renderingInfo);
			...
			vkCmdEndRenderingKHR(drawCommandBuffers[i]);
