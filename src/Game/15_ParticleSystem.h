...
#define PARTICLE_TYPE_SMOKE 1
	...
	void buildCommandBuffers() final
	{
		...
		for (int32_t i = 0; i < drawCommandBuffers.size(); ++i)
		{
			...
			// Environment
			vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets.environment, 0, nullptr);
			vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.environment);
			environment.draw(drawCommandBuffers[i]);

			// Particle system (no index buffer)
			vkCmdBindDescriptorSets(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets.particles, 0, nullptr);
			vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.particles);
			vkCmdBindVertexBuffers(drawCommandBuffers[i], 0, 1, &particles.buffer, offsets);
			vkCmdDraw(drawCommandBuffers[i], static_cast<uint32_t>(particles.size), 1, 0, 0);
	...
	void updateUniformBuffers()
	{
		...
		uniformDataParticles.viewportDim = glm::vec2((float)destWidth, (float)destHeight);
