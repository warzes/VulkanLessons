...
//-----------------------------------------------------------------------------
void GLTFSceneApp::OnWindowResize(uint32_t destWidth, uint32_t destHeight)
{
	if ((destWidth > 0.0f) && (destHeight > 0.0f))
		camera.updateAspectRatio((float)destWidth / (float)destHeight);
}
//-----------------------------------------------------------------------------
void GLTFSceneApp::OnKeyPressed(uint32_t key)
{
	if (key == KEY_F2)
	{
...
//-----------------------------------------------------------------------------
void GLTFSceneApp::OnKeyUp(uint32_t key)
{
	if (camera.type == Camera::firstperson)
	{
		switch (key)
...
//-----------------------------------------------------------------------------
void GLTFSceneApp::OnMouseMoved(int32_t x, int32_t y, int32_t dx, int32_t dy)
{
...
void GLTFSceneApp::loadglTFFile(std::string filename)
{
...
void GLTFSceneApp::loadAssets()
{
	loadglTFFile(getAssetPath() + "models/sponza/sponza.gltf");
}

void GLTFSceneApp::setupDescriptors()
{
	/*
...
void GLTFSceneApp::preparePipelines()
{
	// Layout
...
void GLTFSceneApp::updateUniformBuffers()
{
