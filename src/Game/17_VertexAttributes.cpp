...
//-----------------------------------------------------------------------------
bool VertexAttributesApp::OnCreate()
{
...
//-----------------------------------------------------------------------------
void VertexAttributesApp::OnDestroy()
{
	if (device) {
...
//-----------------------------------------------------------------------------
void VertexAttributesApp::OnUpdateUIOverlay(vks::UIOverlay* overlay)
{
...
//-----------------------------------------------------------------------------
void VertexAttributesApp::OnWindowResize(uint32_t destWidth, uint32_t destHeight)
{
	if ((destWidth > 0.0f) && (destHeight > 0.0f))
		camera.updateAspectRatio((float)destWidth / (float)destHeight);
}
//-----------------------------------------------------------------------------
void VertexAttributesApp::OnKeyPressed(uint32_t key)
{
	if (key == KEY_F2)
	{
...
//-----------------------------------------------------------------------------
void VertexAttributesApp::OnKeyUp(uint32_t key)
{
	if (camera.type == Camera::firstperson)
	{
		switch (key)
...
//-----------------------------------------------------------------------------
void VertexAttributesApp::OnMouseMoved(int32_t x, int32_t y, int32_t dx, int32_t dy)
{
...
//-----------------------------------------------------------------------------
void VertexAttributesApp::loadSceneNode(const tinygltf::Node& inputNode, const tinygltf::Model& input, Node* parent)
{
	Node node{};
...
void VertexAttributesApp::buildCommandBuffers()
{
...
void VertexAttributesApp::loadglTFFile(std::string filename)
{
...
void VertexAttributesApp::uploadVertexData()
{
	// Upload vertex and index buffers

	// Anonymous functions to simplify buffer creation
	// Create a staging buffer used as a source for copies
	auto createStagingBuffer = [this](VulkanBuffer& buffer, void* data, VkDeviceSize size) {
	...
	auto createDeviceBuffer = [this](VulkanBuffer& buffer, VkDeviceSize size, VkBufferUsageFlags usageFlags = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT) {
	...
	*/
	std::array<VulkanBuffer, 4> stagingBuffers;
...
void VertexAttributesApp::loadAssets()
{
	loadglTFFile(getAssetPath() + "models/sponza/sponza.gltf");
}

void VertexAttributesApp::setupDescriptors()
{
...
void VertexAttributesApp::preparePipelines()
{
...
void VertexAttributesApp::updateUniformBuffers()
{
