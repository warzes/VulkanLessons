...
class DeferredMultisamplingApp final : public EngineApp
{
public:
...
private:
	...
	void deferredSetup()
	{
		offscreenframeBuffers = new vks::Framebuffer(m_vulkanDevice);
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCommandBuffers.size(); ++i)
		{
			...
			vkCmdBindPipeline(drawCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, useMSAA ? pipelines.deferred : pipelines.deferredNoMSAA);
	...
	{
		VkSampleCountFlags counts = std::min(m_adapter.deviceProperties.limits.framebufferColorSampleCounts, m_adapter.deviceProperties.limits.framebufferDepthSampleCounts);
